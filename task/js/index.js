...
Rander.prototype = {
	constructor: Rander,
	init: function (fnSubEvent,fnSubEvent2) {
		this.randerBody();
		this.hashChange();
				...
				success: function (json) {
					if ( json.error === 0 ) {
						...
						if ( _this.hash == 1 ) {
							...
							_this.msgObj.removeChild(_this.msgObj.lastChild);
						} else {
	...
	creatEle: function (json) {
		var nowDate = new Date(json.time*1000);
	...
	extend: function() {
		if ( arguments.length === 1 && window.toString.call(arguments[0]) === '[object Object]' ) {
			this.extend(Rander.prototype,arguments[0]);
		}else if (arguments.length === 2) {
			tools.extend(arguments[0],arguments[1]);
		}
	}
}
/*function init() {
...
task.extend(
	{
		fnSubClick:  function() {
			...
			oBtn.addEventListener('click',function(){
				_this.addMsg(oBtn,oText);
			});
		},
		fnSubkeyDown: function() {
			var _this = this;
			oText.addEventListener('keydown',function(ev){
				var ev = ev || event;
				if ( ev.keyCode === 13 ) {
					_this.addMsg(oBtn,oText);
					ev.preventDefault();
