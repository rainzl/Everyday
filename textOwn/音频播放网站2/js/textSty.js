...
/*json = {
	...
	w.fnText = function (cxt,obj,str,startX,startY,json){
		...
		obj.timer = setInterval(function(){
			...
			if ( type == 'normal' ) {
				...
				cxt.fillStyle = backColor;
		 		...
		 		cxt.fillStyle = "#fff";
		 		cxt.fillRect(startX + (x-1)*fontSize ,startY-(fontSize+20),x*fontSize, fontSize+20);
		 	}
		...
		function creatText(txt,x,y,w,h,fontSize) {
			cxt.save();
			cxt.beginPath();
			cxt.fillStyle = backColor;
 			cxt.font = `${fontWeight} ${fontSize}px Arial`;
	...
	for(var i = 0; i<navLi.length; i++){
		...
		navLi[i].children[0].width = aW + 'px';
		navLi[i].children[0].height = aH + 'px';
