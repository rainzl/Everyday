...
//---------------------------------------------------------------------------------分辨率Resolution适配
 ...
 //---------------------------------------------------------------------------------
	...
	var img = ['img/img01.jpg','img/img02.jpg','img/img03.jpg','img/img04.jpg',
				'img/img05.jpg','img/img06.jpg','img/img07.jpg','img/img08.jpg','img/img09.jpg',
				'img/img1.gif','img/img2.gif','img/img3.gif','img/img4.gif','img/img5.gif',
				'img/img6.gif','img/img7.gif','img/img8.gif','img/img9.gif','img/img2.gif',
				'img/img4.gif','img/img8.gif'
			];//21
	//===============
	var ball = 120;
	...
	var nowHash = parseInt(window.location.hash.slice(1));
	var hashArr = [];
		createLi();
		...
		function createLi(){
			for (var i = 0; i < len; i++) {
				...
				li.style.background = "url("+img[nowHash-len + i]+")";//===========可以修改
				a.href= "#"+ (nowHash-len + i);
				//li.innerHTML = nowHash-len + i;
				li.appendChild(a);
			...
			for (var i = 0; i < len; i++) {
				var li = document.createElement('li');
				var a = document.createElement('a');
				li.style.background = "url("+img[nowHash+i+1]+")";//===========可以修改
...
// 左右两ul移入移出各自li位置变化============================
	...
	prev.onclick = function(){
		...
		isTab = false;
		nowHash = nowHash>0?--nowHash:img.length-1;
	...
	function addBall(){
		nowHash = parseInt(window.location.hash.slice(1));
		all.children[0].innerHTML = nowHash;
		all.children[1].innerHTML = img.length-1;
		for (var i = 0; i < len; i++) {
			if(nowHash-len+i < 0){
				lisL[i].style.background = "url("+img[nowHash-len + img.length + i]+")";
				lisL[i].children[0].href= "#"+ (nowHash-len + img.length + i);
			}else{
				lisL[i].style.background = "url("+img[nowHash-len + i]+")";
				lisL[i].children[0].href= "#"+ (nowHash-len + i);
			}
			mTween(lisL[i],{left:0},200,'linear');
		}
		for (var i = 0; i < len; i++) {
			if(nowHash + i+1>img.length-1){
				lisR[i].style.background = "url("+img[nowHash + i+1 - img.length]+")";
				lisR[i].children[0].href= "#"+ (nowHash + i+1- img.length);
			}else{
				lisR[i].style.background = "url("+img[nowHash + i+1]+")";
