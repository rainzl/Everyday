...
/*----------*/
	...
	function clickBalls () {
		...
		function fnOver () {
			if(isgo()){
				...
				onlyRing(zbjArrLise[this.index],zbjCxt,zbjArrLise[this.index].w/2 - 40)
			}
		}
	...
	function onlyRing(item,cxt,val){
		drawEle1(item,cxt,val);
		function drawEle1(item,cxt,val) {
			var w = item.w,
				h = item.h,
				t = item.t,
				l = item.l,
	...
	function drawImg(cxt,x,y,r,img){
		draw(cxt,x,y,r,img)
		function draw(cxt,x,y,r,img){
			cxt.save();
			cxt.beginPath();
			var back= newCanvas(r,img);
			cxt.drawImage(back,x-r, y-r,2*r,2*r)
			cxt.closePath();
			cxt.restore();
		}


		function newCanvas(r,img){
			var newCanvas = document.createElement('canvas');
			newCanvas.width = 2*r,newCanvas.height = 2*r;
			newCanvas.style.left = '100px';
			newCanvas.style.position = 'absolute';
			var newCxt = newCanvas.getContext('2d');
			
			drawImgpik(cxt,r,img)
			return newCanvas;
		}
		function drawImgpik(cxt,r,img) {
			cxt.save();
			cxt.beginPath();
			var img1 = new Image();
			img1.onload = function(){
				cxt.drawImage(img1, 0, 0,2*r,2*r);
				cxt.closePath();
				cxt.restore();
			}
			img1.src = img;
	...
	function getLiSty(search) {
		...
		zbjCxt.style.top = (H-head.offsetHeight-zbjCxt.canvas.height)/2 + H;
		zbjCxt.style.left = zbjCxt.offsetLeft;
		zbjCxt.style.position = 'absolute';
		zbjListA.innerHTML = '';
