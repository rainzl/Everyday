(function(w){
	...
	function setHead(obj) {
		...
		helpBtn.onmouseout = searchSpan.onmouseout = function () {
			var obj  = this.children[0];
		...
		searchClose.onmouseover = function () {
			var obj  = this.children[0];
			shake(obj,'left',1);
		}
		...
		if ( W<=581 || H<=463 ) {/*根据导航padding设置翻页按钮的left值*/
			switchPage.style.left = h1.getBoundingClientRect().left + 'px';
		} else {
			switchPage.style.cssText = '';
		}
