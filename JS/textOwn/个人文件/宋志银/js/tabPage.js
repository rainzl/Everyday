...
function tab(canvas,json,onOff,callBack2) {
	...
	//退出页面
	function goOut() {
		zbjTween(target7,quitData,300,linear,function(){
			if (typeof callBack2 == 'function') {
				zbjTween(quitData,quitTarget,160,linear,callBack2);
			} else {
				zbjTween(quitData,quitTarget,160,linear);
			}
		});
	}
	...
	function draw(arr) {
		...
		if (img) {
			...
			img1.onload = function () {
				...
				cxt.drawImage(img1,-imgW/2,-imgH/2,imgW,imgW);
			...
			cxt.drawImage(cxt2,-cxt2.width/2,-cxt2.height/2,cxt2.width,cxt2.height);
			cxt.globalCompositeOperation = 'destination-atop';
