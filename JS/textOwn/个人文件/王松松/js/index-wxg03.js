(function(w){
	...
	var len = 10;//页面数
	var searchNum = window.location.search.split('=')[1] || 1;
	 window.onresize = function () {
	 	window.location.search = `?list=${searchNum}`;
	 }
	...
	function btnClick(){
		...
		function fnPrev(){
			...
			wss_canvas(zbjCxt,zbjArrLise,defineArr,null,function(){
				searchNum --;
				searchNum = searchNum < 1 ? 1 : searchNum;
				window.location.search = `?list=${searchNum}`;
				prev.addEventListener('click',fnPrev);
			});
		...
		function fnNext(){
			...
			wss_canvas(zbjCxt,zbjArrLise,defineArr,null,function(){
				searchNum ++;
				searchNum = searchNum > len ? len : searchNum;
				window.location.search = `?list=${searchNum}`;
